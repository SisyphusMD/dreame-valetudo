...
class ScriptedConsole(Console):
    ...
    def __init__(self, confirms: list[bool] | None = None, asks: list[str] | None = None,
    ...
    def ask_secret(self, prompt: str) -> str:
        """Captures the question the way confirm does, and never the answer — so a test asserting a
        secret stays out of the transcript is asserting something real."""
...
def make_ctx(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> CtxFactory:
    ...
    def _make(
        *,
        ...
        console = ScriptedConsole(confirms=confirms, asks=asks, secrets=secrets)
