...
def test_the_bottle_wait_outlasts_a_slow_publish() -> None:
    ...
    """
    text = (_ROOT / "packaging" / "build-bottles.sh").read_text()
...
def test_the_install_wait_outlasts_the_bottle_wait_it_depends_on() -> None:
    """The install matrix waits for a release to be installable, and a poured bottle is part of that.
    ...
    independently and the downstream one can expire while the thing it waits for is still
    legitimately coming — reporting a healthy release as a failed one, on both forges at once,
    because they share this script.
    ...
    """
    bottles = (_ROOT / "packaging" / "build-bottles.sh").read_text()
    ...
    wait = (_ROOT / "packaging" / "wait-for-release.sh").read_text()
...
def test_every_apt_base_sets_the_retry_policy() -> None:
    ...
    """
    text = (_ROOT / "packaging" / "install-smoke.Dockerfile").read_text(encoding="utf-8")
...
def test_the_install_matrix_is_reachable_for_every_release() -> None:
    ...
    for forge in (".forgejo", ".github"):
        matrix = yaml.safe_load(
            (_ROOT / forge / "workflows" / "install-matrix.yml").read_text(encoding="utf-8")
        )
    ...
    publish = yaml.safe_load(
        (_ROOT / ".forgejo" / "workflows" / "publish.yml").read_text(encoding="utf-8")
    )
    handoff = [
        job
...
def test_every_supported_version_the_readme_promises_is_install_tested() -> None:
    ...
    pins = (_ROOT / "packaging" / "install-smoke.Dockerfile").read_text(encoding="utf-8")
    ...
    channels = set(
        re.search(r"^CHANNELS=\(\n(.*?)^\)$", (_ROOT / "packaging" / "install-matrix-arch.sh").read_text(encoding="utf-8"), re.M | re.S)
        .group(1)
        .split()
    )
    exercised = set()
...
def test_every_macos_version_the_readme_promises_has_an_install_lane() -> None:
    ...
    matrix = yaml.safe_load(
        (_ROOT / ".github" / "workflows" / "install-matrix.yml").read_text(encoding="utf-8")
    )
...
def test_the_readme_keeps_its_untested_platforms_honest() -> None:
    ...
    floor = (_ROOT / "packaging" / "glibc-floor.txt").read_text(encoding="utf-8").strip()
...
def test_bottles_reach_the_nas_but_stay_out_of_the_quorum() -> None:
    ...
    """
    publish = yaml.safe_load(_PUBLISH.read_text(encoding="utf-8"))
    roles = (_ROOT / "packaging" / "asset-roles.sh").read_text(encoding="utf-8")
