...
#
# A Dockerfile rather than the obvious `docker run`, for the two reasons publish.yml already hit on
# this runner: a plain `docker run --platform arm64` dies with `exec format error` because the host
# has no usable binfmt, and a bind mount of the workspace is invisible to the daemon because the job
# itself runs in a container. BuildKit carries its own QEMU and streams its context, so both problems
# go away — and it is the pattern package-smoke.Dockerfile established here.
#
