...
| `implementation-fix` | H2 | rooted, Valetudo binary installed | Run `fix-impl` on the X40 (and on any other model whose autodetect fails); the live factory config must match the selected workspace before the implementation is written | identity gate and helper succeed; the robot's own config reads back pinned to the live model's implementation |
